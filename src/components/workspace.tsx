...
export default function Workspace() {
  ...
  async function remove(table: "projects" | "tasks" | "snippets", id: string) {
  ...
  async function move(task: Task, status: TaskStatus) {
    ...
    if (result.error) setError(result.error.message);
      ...
      setTasks((list) =>
        list.map((item) => (item.id === task.id ? { ...item, status } : item)),
      );
      notify("Status saved");
    }
  }
        ...
        {projects.length ? (
          projects.map((project) => (
            <article className="record-card" key={project.id}>
              ...
              <button onClick={() => onEdit("projects", project.id)}>
                Edit
              </button>
              <button
                className="row-menu"
...
function Editor({
  ...
  if (table === "projects")
    return (
      <ProjectForm existing={existing as Project | undefined} onDone={onDone} />
    );
  if (table === "tasks")
    return (
      <TaskForm
  ...
  return (
    <SnippetForm existing={existing as Snippet | undefined} onDone={onDone} />
  );
}
  ...
  return (
    <div className="panel">
      ...
      <div className="github-actions">
        <button className="text-button" onClick={() => void load("repos")}>
          Load repositories
        </button>
        <button className="text-button" onClick={() => void load("activity")}>
          Load activity
        </button>
      </div>
      {data.length > 0 && (
        <button
          ...
          onClick={() => {
            ...
            if (first.full_name) void load("commits", first.full_name);
          }}
        >
